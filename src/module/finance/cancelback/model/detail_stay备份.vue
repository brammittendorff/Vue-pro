  ...
  methods: {
    ...
    handCardel(val) {
      ...
      this.checkAllSelect.map(el => {
        if (this.citiesSelect.includes(el)) {
          arr.push(el);
        }
      });
      // 判断全选按钮是全选还是半选还是不选
      if (arr && arr.length == this.citiesSelect && this.citiesSelect.length) {
    ...
    //   点击显示选号车
    handStayCar() {
      // this.stayCarShow = true;
      console.log(document.querySelectorAll('.icon-arrow-down'),this.stayCarShow)
      if(this.stayCarShow){
        document.querySelectorAll('.icon-arrow-down')[0].setAttribute('style','transform:rotate(-180deg);')
        return
      }
      document.querySelectorAll('.icon-arrow-down')[0].setAttribute('style','transform:rotate(0deg);')
    },
    //多选框
    ...
    //   if (val) {
    //     this.checkAllSelect.push(...this.citiesSelect);
    //     this.checkAllSelect = [...new Set(this.checkAllSelect)];
    //   } else {
    //     this.citiesSelect.map(el => {
    //       this.checkAllSelect.splice(this.checkAllSelect.indexOf(el), 1);
    //     });
    //   }
    //   console.log(val, this.checkAllSelect, "//1212121212///.//");
    //   this.isIndeterminate = false;
    // },
    // 单选
    ...
    getCheckData(data) {
      ...
      data.map(el => {
        console.log(el.id, "////./././");
        // this.cities.push(el.id); // 翻页是的所有数据id
        this.citiesSelect.push(el.id); //   当前页的数据id
      });
      ...
      //   // 判断
      //   if (this.checkAllSelect.includes(this.citiesSelect[i])) {
      //     arr.push(this.citiesSelect[i]);
      //   }
      // }
      // 当前页选中的数据  防止翻页时丢失
      // console.log(this.checkAllSelect, arr);
      this.checkAllSelect = [...new Set(this.checkAllSelect)];
      console.log(this.checkAllSelect, "/this.");
      // 判断全选按钮是否选中
      // if (arr.length == 0) {
      //   this.isIndeterminate = false;
      //   this.checkAll = false;
      // } else if (arr.length == this.citiesSelect.length) {
      //   this.isIndeterminate = false;
      //   this.checkAll = true;
      // } else {
      //   this.isIndeterminate = true;
