

<template>
    <div class="newaddnum g-box-shadow">
        <header>
            <p style="display:flex;align-items:center"><span class="dot g-fl"></span>&ensp;<span style='font-size:14px'>新增媒体主数量(已有联系人)</span>
            <el-select v-model="year" placeholder="请选择年份" size="mini" @change="getAddData">
            <el-option v-for="(item,i) in yearData" :key="i" :label="item" :value="item"></el-option>
        </el-select>

        <el-select v-model="month" clearable placeholder="请选择月份" size="mini" @change="getAddData"> 
            <el-option label="一月" :value="1"></el-option>
            <el-option label="二月" :value="2"></el-option>
            <el-option label="三月" :value="3"></el-option>
            <el-option label="四月" :value="4"></el-option>
            <el-option label="五月" :value="5"></el-option>
            <el-option label="六月" :value="6"></el-option>
            <el-option label="七月" :value="7"></el-option>
            <el-option label="八月" :value="8"></el-option>
            <el-option label="九月" :value="9"></el-option>
            <el-option label="十月" :value="10"></el-option>
            <el-option label="十一月" :value="11"></el-option>
            <el-option label="十二月" :value="12"></el-option>
        </el-select></p>
        </header>
        <div class="conter" v-loading="loading">
            <!-- <p>{{titleText}}</p> -->
            <div><echarts id='articleData1' :style="{width: '100%', height: '400px'}" :option='echartsOption1' v-if="!loading"/></div>
        </div>
    </div>
</template>


<script>
import Echarts from "@/components/echarts/index";
export default {
   components: {
    Echarts,
  },
  data() {
   
    return {
        yearData:null,
        loading:true,
        getDate:'',
        year:'',
        month:'',
      echartsOption1: {
        color:[
          "#2ec7c9",
          "#b6a2de",
          "#6495ed",
          ],
        title : {
        text: '媒体主数（已有联系人）',
        subtext: '媒体主数/个',
        textStyle: {
            color: "#1192dd",
            fontSize: "16",
            fontFamily: "微雅软黑"
        }
    },
    tooltip : {
        trigger: 'axis'
    },
    legend: {
        data:['5w粉以下','5w-30w粉','30w粉以上']
    },
    toolbox: {
        show : true,
        color:['#37A2DA','#f7A2DA'],
        feature : {
            // dataView : {show: true, readOnly: false},
            magicType : {show: true, type: ['line', 'bar']},
            // restore : {show: true},
            // saveAsImage : {show: true}
        },
        // iconStyle:{
        //     normal:{
        //         color:'red',//设置颜色
        //     }
        // }
    },
    // calculable : true,
    xAxis : [
        {
            type : 'category',
            data : []
        }
    ],
    yAxis : [
        {
            type : 'value'
        }
    ],
    series : [
        {
            name:'5w粉以下',
            type:'bar',
            data:[],
            markPoint : {
                data : [
                    {type : 'max', name: '最大值'},
                    {type : 'min', name: '最小值'}
                ]
            },
            markLine : {
                data : [
                    {type : 'average', name: '平均值'}
                ]
            },
            itemStyle:{
                normal:{
                    barBorderRadius:[5,5,5,5]
                }
            },
            smooth:true
        },
        {
            name:'5w-30w粉',
            type:'bar',
            data:[],
            markPoint : {
                data : [
                    {name : '年最高', type:'max'},
                    {name : '年最低', type:'min'}
                ]
            },
            markLine : {
                data : [
                    {type : 'average', name : '平均值'}
                ]
            },
            itemStyle:{
                normal:{
                    barBorderRadius:[5,5,5,5]
                }
            },
            smooth:true
        },
        {
            name:'30w粉以上',
            type:'bar',
            data:[],
            markPoint : {
                data : [
                    {name : '年最高', type:'max'},
                    {name : '年最低', type:'min'}
                ]
            },
            markLine : {
                data : [
                    {type : 'average', name : '平均值'}
                ]
            },
            itemStyle:{
                normal:{
                    barBorderRadius:[5,5,5,5]
                }
            },
            smooth:true
        }
    ]
      }
    }
  },
  methods: {
    // 获取年份
    getYear(){
    let date = new Date()
    let year = date.getFullYear()
    this.yearData=[year]
    for (let i = 0; i < year-2015; i++) {
        this.yearData.push(date.getFullYear()-i-1)
    }
    },
      getAddData() {
          this.loading=true
              let postval={
                  year:this.year,
                  month:this.month
              }
              this.$axios.get('/media_analysis/get_media_increasing_statistics',{params:postval}).then(e=>{
                  this.echartsOption1.series[0].data=[]
                  this.echartsOption1.series[1].data=[]
                  this.echartsOption1.series[2].data=[]
                  this.echartsOption1.xAxis[0].data=[]
                  if(e.data.code==200){
                      this.loading=false
                      let newCustomer=[],
                      oldCustomer=[],
                      allCustomer=[],
                        xAxis=[]
                      e.data.data.forEach(element => {
                          newCustomer.push(element.media_count_level_1)
                          oldCustomer.push(element.media_count_level_2)
                          allCustomer.push(element.media_count_level_3)
                          if(this.month){
                              xAxis.push(element.week)
                          }else{
                              xAxis.push(element.month)
                          }
                          
                      });
                      this.echartsOption1.series[0].data=newCustomer
                      this.echartsOption1.series[1].data=oldCustomer
                      this.echartsOption1.series[2].data=allCustomer
                      this.echartsOption1.xAxis[0].data=xAxis
                  }else{
                      this.$message.error(e.data.code)
                      this.echartsOption1.series[0].data=[]
                      this.echartsOption1.series[1].data=[]
                      this.echartsOption1.series[2].data=[]
                      this.loading=false
                  }
              }).catch(err=>{
                  this.loading=false
                //   this.$message.error('网络错误')
              })
      }
  },
  mounted(){
      this.getDate=new Date()
      this.year=this.getDate.getFullYear()
      this.getAddData()
      this.getYear()
  }
}
</script>

<style lang="less" scoped>
 .newaddnum {
  padding: 20px 30px;
  margin-top: 20px;
  background-color: #fff;
  border: 1px solid #eee;
  header {
    padding: 0px 0px 0px 10px;
    margin-bottom: 15px;
    display: flex;
    .dot {
      width: 1px;
      height: 20px;
      background: #ff8800;
      border-radius: 2px;
      border: 2px solid #ff8800;
    }
    span{
      line-height: 20px;
    }
    .el-select {
      padding-left: 10px;
    }
  }
  .conter {
    display: flex;
    justify-content: space-between;
    width: 100%;
    position: relative;
    padding-top: 15px;
    border-top: 1px solid rgb(225, 228, 230);
    > section {
      position: absolute;
      right: 11%;
    }
    > div {
      width: 100%;
      min-height: 400px;
    }
    >p{
        position: absolute;
      left:0px;
      color: #1192dd;
      font-size: 18px;
      font-family: "微雅软黑";
    }
  }
}
</style>
